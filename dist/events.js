 ...
 */
export function eventPool(live) {
 ...
 */
export function sideOf(event, live) {
    if (event.team === 'home' || event.team === 'away')
        return event.team;
    const short = upper(event.teamShort);
    if (short) {
        if (short === upper(live?.home?.short))
            return 'home';
        if (short === upper(live?.away?.short))
            return 'away';
    }
    const name = upper(event.teamName);
    if (name) {
        if (name === upper(live?.home?.name))
            return 'home';
        if (name === upper(live?.away?.name))
            return 'away';
    }
    return null;
}
...
export function redCards(live) {
    ...
    for (const event of eventPool(live)) {
        ...
        if (which)
            tally[which] = Math.min(tally[which] + 1, MAX_RED_CARDS);
    }
    return tally;
}
