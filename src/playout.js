...
function suppressed() {
  ...
  for (const layer of LAYERS) {
    ...
    if (!entry) continue;
    for (const target of byId(entry.id)?.hides || []) hidden.add(target);
  }
  ...
  for (const layer of LAYERS) {
    ...
    if (host && hidden.has(host.layer)) hidden.add(def.layer);
  }
...
function take(id, data) {
  const def = byId(id);
  if (!def) return { ok: false, error: `No graphic named ${id}`, reason: 'invalid' };
  ...
  if (replaced) clearTimer(replaced.id);
  clearTimer(id);
  ...
  if (def.autoOut && timersEnabled) {
    ...
    timers.set(
      ...
      setTimeout(() => {
        ...
        if (current && current.seq === firedFor) clear(id);
      }, def.autoOut),
    );
  }
...
function clear(id) {
  ...
  clearTimer(id);
  if (layers[def.layer]?.id !== id) return { ok: true, state: state() };
  ...
  for (const child of docked(id)) {
    if (layers[child.layer]?.id !== child.id) continue;
    clearTimer(child.id);
...
function panic() {
  ...
  for (const layer of LAYERS) layers[layer] = null;
  seq += 1;
  panicSeq = seq;
...
function templates(ids) {
  ...
  if (gap.length) {
    console.error(
      `[graphics] the render window has no template for: ${gap.join(', ')} — ` +
...
function phase(id, value) {
  if (value === 'out') delete actual[id];
  else actual[id] = value;
  announce();
}
...
function hydrate(next, { timers: useTimers = false } = {}) {
  ...
  for (const layer of LAYERS) layers[layer] = incomingLayers[layer] ?? null;
  for (const id of Object.keys(actual)) delete actual[id];
  ...
  if (useTimers) {
    for (const layer of LAYERS) {
      const entry = layers[layer];
      if (!entry || !entry.autoOutAt) continue;
      const firedFor = entry.seq;
      const id = entry.id;
      timers.set(
        id,
