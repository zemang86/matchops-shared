...
export const STOPPED: ClockState = { running: false, baseSeconds: 0, since: null };
 ...
 */
export function secondsOf(state: ClockState | null | undefined, now = Date.now()): number {
  if (!state) return 0;
...
export function play(state: ClockState, now = Date.now()): ClockState {
  if (state.since !== null) return { ...state, running: true };
  return { running: true, baseSeconds: state.baseSeconds, since: now };
}

export function pause(state: ClockState, now = Date.now()): ClockState {
  if (state.since === null) return { ...state, running: false };
...
export function set(
  ...
  seconds: number,
  now = Date.now(),
): { ok: true; state: ClockState } | { ok: false; error: string } {
